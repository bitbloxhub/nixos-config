-- From https://github.com/willothy/nvim-config/blob/f747425/lua/resession/extensions/edgy.lua
...
local function save_win(winid)
	...
	if win and win.view.open then
		...
		return {
			...
			tabpage = vim.api.nvim_tabpage_get_number(vim.api.nvim_win_get_tabpage(winid)),
		}
	end
end
...
function M.on_post_load(data)
	...
	for _, win_info in ipairs(data.wins or {}) do
		...
		if f and (data.scope == "tabpage" or tabs[win_info.tabpage + first_tab]) then
			...
			vim.api.nvim_win_call(vim.api.nvim_tabpage_get_win(tab), function()
				if not pcall(f) then
					vim.schedule(function()
						vim.notify("[resession] Failed to open " .. win_info.ft, vim.log.levels.WARN)
					end)
				end
			end)
		end
