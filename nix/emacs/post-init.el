...
(use-package
 ...
 :commands
 (undo-fu-only-undo
  undo-fu-only-redo undo-fu-only-redo-all undo-fu-disable-checkpoint)
 :config
...
(use-package
 ...
 (define-key
  my/notes-prefix-map (kbd "j") #'my/org-roam-dailies-open-today)
 (define-key
  my/notes-prefix-map (kbd "D") #'org-roam-dailies-goto-date)
 :config
 ...
 (add-hook
  'org-after-refile-insert-hook #'my/org-roam-db-update-current-file)
 (add-to-list
  'display-buffer-alist
  '("\\*Org Select\\*" (display-buffer-below-selected)))
 (add-to-list
  'display-buffer-alist '("CAPTURE*" (display-buffer-same-window))))
...
(use-package
 ...
 :init (global-set-key (kbd "C-'") 'avy-goto-char-2))
...
;; Configure the `tab-bar-show` variable to t to display the tab bar always
